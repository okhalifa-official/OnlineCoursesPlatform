    ...
    <section className="max-w-6xl mx-auto px-6 pt-16 pb-20 flex flex-col lg:flex-row items-center gap-12">
      ...
      <div className="flex-1 max-w-xl">
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-brandRed text-xs font-bold uppercase tracking-widest">
            {hero?.badge ?? "Internationally Accredited POCUS"}
          </span>
        </div>
        ...
        <h1 className="font-heading font-black text-charcoal leading-tight mb-5" style={{ fontSize: "clamp(2rem, 4vw, 3rem)" }}>
          {hero ? (
            <>
              {hero.headline.replace(hero.headlineHighlight, "").split(hero.headlineHighlight)[0]}
              <span className="text-brandRed">{hero.headlineHighlight}</span>
              {hero.headline.split(hero.headlineHighlight)[1]}
            </>
          ) : (
            <>Hands-on Ultrasound <span className="text-brandRed">Training</span> for the Real Bedside.</>
          )}
        </h1>
      ...
      <div className="flex-1 flex flex-col items-end gap-4">
        <div className="flex items-center gap-2 self-center mb-1">
          ...
          <span className="font-bold text-charcoal text-sm">{hero?.rating.value} / 5</span>
          <span className="text-gray-400 text-xs">{hero?.rating.reviews} reviews</span>
        </div>
        ...
        <div className="bg-charcoal text-white rounded-xl px-4 py-3 text-xs self-end">
          <p className="font-semibold">{hero?.workshopBadge.title}</p>
          <p className="text-gray-400 mt-0.5">{hero?.workshopBadge.subtitle}</p>
        </div>
      </div>
    </section>
  );
